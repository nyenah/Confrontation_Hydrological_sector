...
class Confrontation(object):
    ...
    def modelPlots(self,m):
        ...
        with Dataset(fname) as dataset:
            ...
            """Note for NSE or KGE based on annual cycle, change  'spaceint' which is the same as
               the regional mean  to 'cycle' in line 712-713 (1st word in variable name) """
            model_dis = Variable(filename=fname,groupname="MeanState",variable_name='spaceint_of_'+self.variable+'_over_global')
            obs_dis = Variable(filename=bname,groupname="MeanState",variable_name='spaceint_of_'+self.variable+'_over_global').convert(model_dis.unit)        
            obs_mean=obs_dis.integrateInTime(mean=True)
            model_mean=model_dis.integrateInTime(mean=True)
            
            #NSE 
            diff_num = deepcopy(model_dis) 
            diff_num.data = (model_dis.data-obs_dis.data)**2 
            diff_num=diff_num.integrateInTime()
            diff_den = deepcopy(obs_dis)
            diff_den.data = (obs_dis.data-obs_mean.data)**2
            diff_den=diff_den.integrateInTime()
            NSE=1-(diff_num.data/diff_den.data)
            print('NSE'+'_'+self.name+'_'+m.name+'='+str(NSE))
                        
            #KGE
            r=obs_dis.correlation(var=model_dis,ctype='temporal', region=None)
            """Internannual variability in this case is defined as the standard deviation of the 
            data in the temporal dimension. for more info See ILAMB documentation"""            
            obs_std=obs_dis.interannualVariability() 
            mod_std=model_dis.interannualVariability()
            KGE=1-((r.data-1)**2+((mod_std.data/obs_std.data)-1)**2+((model_mean.data/obs_mean.data)-1)**2)**0.5
            print('KGE'+'_'+self.name+'_'+m.name+'='+str(KGE)) 
            
            """
            #just values to calculate the above metrics 
            print('Statistics')
            print('correlarion ='+' '+str(r.data))
            print('Obs_mean ='+' '+str(obs_mean.data))
            print('model_mean ='+' '+str(model_mean.data))
            print('Obs_std ='+' '+str(obs_std.data))
            print('model_std ='+' '+str(mod_std.data))"""
